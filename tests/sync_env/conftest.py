...
import pytest
from sqlalchemy import create_engine, DateTime, Engine, func, Integer, String, Text, URL
...
def sync_engine_() -> Engine:
    ...
    sync_engine_ = create_engine(DB_URL, echo=True)
...
@pytest.fixture(scope="function", autouse=True)
def transaction(session_factory_, session_start_up) -> Session:
    scoped_session_ = scoped_session(session_factory_)
